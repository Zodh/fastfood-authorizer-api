...
resource "aws_lambda_function" "eks_invoker" {
  ...
  environment {
    variables = {
      EXTERNAL_IP_ORDER  = "http://${data.kubernetes_service.order_service.status[0].load_balancer[0].ingress[0].hostname}:80"
      EXTERNAL_IP_PERSON  = "http://${data.kubernetes_service.person_service.status[0].load_balancer[0].ingress[0].hostname}:80"
